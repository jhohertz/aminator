...
"""
aminator.plugins.volume.one_part
=============================
basic one partition volume allocator
"""
import logging
import os
...
__all__ = ('OnePartVolumePlugin',)
log = logging.getLogger(__name__)


class OnePartVolumePlugin(BaseVolumePlugin):
    _name = 'one_part'

    def _attach(self, blockdevice):
        with blockdevice(self._cloud) as dev:
            self._dev = dev
            self._rootdev = "{0}1".format(self._dev)
            # FIXME: unsure what context this needs yet.
    ...
    def _mount(self):
        ...
        self._mountpoint = os.path.join(self._volume_root, os.path.basename(self._rootdev))
        if not os.path.exists(self._mountpoint):
            os.makedirs(self._mountpoint)

        if not mounted(self._mountpoint):
            mountspec = MountSpec(self._rootdev, None, self._mountpoint, None)
    ...
    def _unmount(self):
        if mounted(self._mountpoint):
            if busy_mount(self._mountpoint).success:
                raise VolumeException('Unable to unmount {0} from {1}'.format(self._rootdev, self._mountpoint))
            result = unmount(self._mountpoint)
            if not result.success:
                raise VolumeException('Unable to unmount {0} from {1}: {2}'.format(self._rootdev, self._mountpoint,
